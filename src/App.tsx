...
import { syncSubscriptionPlan, isPremiumActive, grantWelcomeTrial } from './services/promo';
...
import {
  ...
  getDueReminders, markReminderSent, getCountdown, formatCountdown
...
import { creditWorkout, formatCooldown, msUntilNextCredit } from './services/workoutLog';
...
export default function App(): JSX.Element {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      if (activeSession) setCurrentUserId(activeSession.firebaseUid ?? activeSession.id);
      ...
      if (session) {
        ...
        // VK ID users or returning sessions without premiumUntil get their 30-day trial automatically
        if (session.provider === 'vk' && !profile.premiumUntil) {
          profile = grantWelcomeTrial(profile);
        }
        if (freshAccount) {
      ...
      // Keep the final clean baseline (including the 30-day welcome trial)
      // in the local mirror and Firestore.
      if (freshAccount) persistFreshProfile(profile);
  ...
  useEffect(() => {
    ...
    // Deep Link Listener for opening trainings (#training=ID)
    const removeLinkListener = setupDeepLinkListener((trainingId) => {
  ...
  const handleSendChatMessage = () => {
    ...
    sendChatMessage(openChatThread.id, openChatCompanion.id, chatDraft);
    setChatDraft('');
    setChatThreads(loadChatThreads(currentUser, allUsers, chatCategory));
  };
  ...
  const handleCreatePost = async () => {
    ...
    const post = await createPost(postContent, postSportTag, media);
    if (post) {
  ...
  const handleToggleEventRegistration = (event: OfficialEvent) => {
    if (!currentUser) return;
    const updated = toggleEventRegistration(event.id, currentUser.id);
    if (updated) {
    ...
    <div className="relative min-h-[100svh] bg-slate-950 text-slate-100 pb-24 font-sans antialiased">
          ...
          >
            ...
            {activeTab === 'profile' && currentUser && (
              <div className="space-y-5 pb-6">
                ...
                {/* Search athletes by first name / last name */}
                <button
                  type="button"
                  onClick={() => {
                    triggerHapticImpact('light');
                  ...
                  className="w-full py-3.5 bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-emerald-500/40 text-slate-200 font-bold rounded-2xl text-xs transition active:scale-[0.99] flex items-center justify-center gap-2"
                >
                  <UserPlus className="w-4 h-4 text-emerald-400" />
                  Найти человека по имени или фамилии
                </button>
      ...
      <UserSearchModal
        ...
        onFriendAdded={(userId) => {
          setCurrentUser((prev) => prev ? { ...prev, friendIds: [...new Set([...(prev.friendIds || []), userId])] } : prev);
          setAllUsers((prev) => prev.map((u) => u.id === CURRENT_USER_ID
            ? { ...u, friendIds: [...new Set([...(u.friendIds || []), userId])] }
            : u));
        }}
      />
            ...
            {currentUser?.isVerified ? (
              ...
              <>
                ...
                <p className="mt-1.5 text-xs leading-relaxed text-slate-300">
                  ...
                  призовые BOX и все возможности сообщества.
                </p>
              </>
            )}
          </div>
