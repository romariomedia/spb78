...
/** Silent anonymous session for the Firestore transport layer. */
export async function ensureAnonymousSession(): Promise<FirebaseUser | null> {
  if (currentUser) return currentUser;
  try {
    const result = await Promise.race([
      signInAnonymously(auth),
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error('auth-timeout')), SIGN_IN_TIMEOUT_MS)
      )
    ]);
