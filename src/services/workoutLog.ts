...
/** Firestore is authoritative; localStorage is only a read cache. */
export async function refreshWorkoutCredits(userId: string): Promise<WorkoutCredit[]> {
  if (!userId) return [];
  const snap = await getDocs(query(collection(db, 'workoutCredits'), where('userId', '==', userId)));
  const credits = snap.docs.map(d => d.data() as WorkoutCredit).sort((a,b) => b.timestamp - a.timestamp);
  writeLog([...credits, ...readLog().filter(c => c.userId !== userId)]);
  return credits;
}
...
export async function creditWorkout(
  user: UserProfile, source: CreditSource, training: Training
): Promise<CreditResult> {
  if (!hasValidTrainingDate(training)) return { ok:false, reason:'missing-training-date', user, total:user.totalWorkouts };
  if (getTrainingDayKey(training) !== getDayKey()) return { ok:false, reason:'not-training-day', user, total:user.totalWorkouts };
  if (!training.isCompleted) return { ok:false, reason:'training-not-completed', user, total:user.totalWorkouts };
  if (source === 'organizer-completion' && training.createdBy !== user.id) return { ok:false, reason:'not-organizer', user, total:user.totalWorkouts };
  if (source === 'participant-completion' && (!training.participantIds.includes(user.id) || training.createdBy === user.id)) return { ok:false, reason:'not-registered', user, total:user.totalWorkouts };
  try {
    const result = await callServer<{ total:number; credit:WorkoutCredit }>('/api/sportbuddy-mutation', { action:'workoutCredit', trainingId:training.id, source });
    const log = readLog().filter(c => c.id !== result.credit.id); writeLog([result.credit, ...log]);
    const updated = { ...user, totalWorkouts: result.total }; triggerHapticImpact('medium'); triggerHapticNotification('success');
    return { ok:true, user:updated, total:result.total, credit:result.credit };
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Не удалось засчитать тренировку';
    if (/сегодня|уже засчит/i.test(message)) return { ok:false, reason:'already-credited', user, total:user.totalWorkouts, cooldownMs:msUntilNextCredit(), credit:getTodayCredit(user.id) };
    if (/присутств/i.test(message)) return { ok:false, reason:'not-checked-in', user, total:user.totalWorkouts };
    throw error;
  }
}
