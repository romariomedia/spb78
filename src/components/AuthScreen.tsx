...
import { getHighQualityVKAvatar } from '../services/vkid';
import { triggerHapticImpact } from '../services/native';

const VK_APP_ID = 54699979;
const VK_REDIRECT_URL = 'https://sportbuddy78.pro';
...
export function AuthScreen({ onAuthenticated, initialNotice = '' }: Props) {
  ...
  useEffect(() => {
    ...
    let removeVkCallback: (() => void) | undefined;
    try {
      VKID.Config.init({
        app: VK_APP_ID,
      ...
      // Listen for VK OAuth callback from deep link
      const handleVkCallback = (e: Event) => {
        ...
        void finishVkLogin(code, device_id);
      };
      window.addEventListener('vk-oauth-callback', handleVkCallback);
      removeVkCallback = () => window.removeEventListener('vk-oauth-callback', handleVkCallback);
