...
3. VK ID перенаправляет браузер на `https://sportbuddy78.pro` с кодом авторизации в URL.
...
<activity
    ...
    <!-- ДОБАВИТЬ ЭТО: Deep Link для VK ID OAuth -->
...
- `android:pathPrefix="/"` — всё, что начинается с корня домена.
...
4. VK ID перенаправляет: `https://sportbuddy78.pro/?code=xxx&device_id=yyy`
